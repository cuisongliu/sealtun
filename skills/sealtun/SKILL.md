...
- User operation: install, shell completion, login, expose HTTPS, SSH, or generic TCP, secure public HTTP traffic, bind a domain, inspect state, stop/start/resume, clean up, or use the dashboard. Read `references/cli.md`.
...
5. Control mutations. Do not run `sealtun expose`, `sealtun apply`, `sealtun domain set/clear`, `sealtun stop`, `sealtun cleanup`, or `sealtun logout` unless the user explicitly asked for that operation in the current task. For declarative changes, prefer `apply --dry-run` and `diff` before real `apply`.
