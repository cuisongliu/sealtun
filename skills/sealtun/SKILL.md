...
- User operation: install, shell completion, login, expose HTTPS, SSH, or generic TCP, generate protocol templates, secure public HTTP traffic, create/list/revoke temporary share links, plan/add/verify a custom domain, inspect state, stop/start/resume, clean up, export YAML, or use the dashboard. Read `references/cli.md`.
...
- For dashboard remote access, recommend `--basic-auth-user` plus `--basic-auth-password-env` with `--allow-remote`; `--open` is useful for local loopback dashboards.
